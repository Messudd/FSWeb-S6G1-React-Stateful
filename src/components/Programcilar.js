...
export default function Programcilar() {
  ...
  const [ProgramcilarListesi , setProgramcilarListesi] = useState(enIyilerListesi);
  const [oneCikanID, setOneCikanID] = useState("");

	
  const oneCikaninIsmi = () => {

    const devName = ProgramcilarListesi[oneCikanID-1].isim;
    return devName;
  ...
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: oneCikanID ? 'gold' : 'royalblue', // royalblue', // 🤔 kutlarken renk gold'a dönecek
  };
